...
DEFAULT_DATA = {
    "settings": {
        ...
        "panel_styles": ["Default", "Dark", "Light", "Angry", "Whisper"],
        "panel_moving_options": ["None", "Ping-pong", "Shake", "Float", "Pulse"],
        "godot_exe": "",
        "godot_project_root": "",
        "resource_output_path": "res://dialogue_box_generated.tres"
    },
    "entries": []
}
...
class DialogueEditor(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Silent Tears Dialogue Editor v5")
    ...
    def _build_ui(self):
        ...
        self.save_btn = QPushButton("Save + Export Resource")
        self.save_as_btn = QPushButton("Save As + Export")
        ...
        for btn in [self.add_entry_btn, self.duplicate_entry_btn, self.delete_entry_btn, self.move_up_btn, self.move_down_btn]:
        ...
        desc = QLabel(
            "Editor leírás: itt egy dialogue entry-t szerkesztesz. "
            "A seen_entry nincs kitéve, mert azt a játék runtime kezeli. "
            "A speaker, panel_moving és panel_style mezők mellett + gomb van, "
            "amivel azonnal új opciót adhatsz hozzá."
        )
        ...
        export_group = QGroupBox("Godot auto resource export")
        export_layout = QVBoxLayout(export_group)

        export_layout.addWidget(QLabel("Godot exe path"))
        godot_row = QHBoxLayout()
        self.godot_exe_line = QLineEdit()
        self.godot_exe_btn = QPushButton("Browse")
        self.godot_exe_line.textChanged.connect(self.update_export_settings)
        self.godot_exe_btn.clicked.connect(self.browse_godot_exe)
        godot_row.addWidget(self.godot_exe_line)
        godot_row.addWidget(self.godot_exe_btn)
        export_layout.addLayout(godot_row)

        export_layout.addWidget(QLabel("Godot project root - ahol a project.godot van"))
        project_row = QHBoxLayout()
        self.project_root_line = QLineEdit()
        self.project_root_btn = QPushButton("Browse")
        self.project_root_line.textChanged.connect(self.update_export_settings)
        self.project_root_btn.clicked.connect(self.browse_project_root)
        project_row.addWidget(self.project_root_line)
        project_row.addWidget(self.project_root_btn)
        export_layout.addLayout(project_row)

        export_layout.addWidget(QLabel("Resource output path - root mentéshez hagyd így, vagy nevezd át"))
        self.resource_output_line = QLineEdit("res://dialogue_box_generated.tres")
        self.resource_output_line.textChanged.connect(self.update_export_settings)
        export_layout.addWidget(self.resource_output_line)

        self.export_now_btn = QPushButton("Run Godot Export Now")
        self.export_now_btn.clicked.connect(self.export_resource_from_current_json)
        export_layout.addWidget(self.export_now_btn)

        info = QLabel(
            "Mentés után automatikusan lefut, ha be van állítva a Godot exe és a project root. "
            "A script bemásolja az export resource scripteket a projektedbe: "
            "res://addons/dialogue_python_exporter/"
        )
    ...
    def _set_editor_enabled(self, enabled):
        for widget in [
            self.speaker_combo, self.add_speaker_btn_top,
            self.content_edit, self.color_line, self.color_btn,
            self.text_speed_spin,
            self.panel_moving_combo, self.add_moving_btn_top,
            self.panel_style_combo, self.add_style_btn_top
        ]:
    ...
    def _refresh_export_ui(self):
        self.godot_exe_line.blockSignals(True)
        self.project_root_line.blockSignals(True)
        self.resource_output_line.blockSignals(True)

        self.godot_exe_line.setText(self.data["settings"].get("godot_exe", ""))
        self.project_root_line.setText(self.data["settings"].get("godot_project_root", ""))
        self.resource_output_line.setText(self.data["settings"].get("resource_output_path", "res://dialogue_box_generated.tres"))

        self.godot_exe_line.blockSignals(False)
        self.project_root_line.blockSignals(False)
        self.resource_output_line.blockSignals(False)
    ...
    def _normalize_loaded_data(self, loaded):
        ...
        data["settings"].setdefault("godot_exe", "")
        data["settings"].setdefault("godot_project_root", "")
        data["settings"].setdefault("resource_output_path", "res://dialogue_box_generated.tres")
    ...
    def save_file(self):
        ...
        if self._save_to_path(self.current_file):
            self.export_resource_from_current_json()
    ...
    def update_export_settings(self):
        ...
        self.data["settings"]["godot_exe"] = self.godot_exe_line.text()
        self.data["settings"]["godot_project_root"] = self.project_root_line.text()
        self.data["settings"]["resource_output_path"] = self.resource_output_line.text()

        print_log("Export settings updated")

    def browse_godot_exe(self):
        ...
        if path:
            self.godot_exe_line.setText(path)
            self.update_export_settings()

    def browse_project_root(self):
        path = QFileDialog.getExistingDirectory(self, "Select Godot project root")

        if path:
            self.project_root_line.setText(path)
            self.update_export_settings()

    def _install_godot_exporter_files(self, project_root):
        target_dir = os.path.join(project_root, "addons", "dialogue_python_exporter")
        os.makedirs(target_dir, exist_ok=True)

        source_dir = os.path.join(app_dir(), "godot_exporter_files")

        if not os.path.isdir(source_dir):
            raise RuntimeError(f"Missing godot_exporter_files folder next to dialogue_editor.py: {source_dir}")

        for filename in [
            "json_to_resource_exporter.gd",
            "st_dialogue_box.gd",
            "st_dialogue_entry.gd",
            "st_dialogue_style.gd"
        ]:
            src = os.path.join(source_dir, filename)
            dst = os.path.join(target_dir, filename)

            shutil.copyfile(src, dst)
            print_log(f"Copied exporter file: {dst}")

        return "res://addons/dialogue_python_exporter/json_to_resource_exporter.gd"

    def export_resource_from_current_json(self):
        if not self.current_file:
            print_log("Export skipped: no current JSON file")
            return

        self.update_export_settings()

        godot_exe = self.data["settings"].get("godot_exe", "").strip()
        project_root = self.data["settings"].get("godot_project_root", "").strip()
        output_path = self.data["settings"].get("resource_output_path", "res://dialogue_box_generated.tres").strip()

        if not godot_exe:
            print_log("Godot export skipped: Godot exe path is empty")
            return

        if not os.path.isfile(godot_exe):
            print_log(f"Godot export skipped: Godot exe does not exist: {godot_exe}")
            return

        if not project_root or not os.path.isdir(project_root):
            print_log(f"Godot export skipped: invalid project root: {project_root}")
            return

        if not os.path.isfile(os.path.join(project_root, "project.godot")):
            print_log(f"Godot export skipped: project.godot not found in: {project_root}")
            return

        if not output_path.startswith("res://"):
            print_log("Output path was not res://, changed to res://dialogue_box_generated.tres")
            output_path = "res://dialogue_box_generated.tres"
            self.resource_output_line.setText(output_path)
            self.data["settings"]["resource_output_path"] = output_path

        try:
            exporter_res_path = self._install_godot_exporter_files(project_root)
            ...
            print_log("Running Godot exporter:")
            print_log(" ".join(cmd))
            ...
            print_log("Godot exporter stdout:")
            print(result.stdout)

            print_log("Godot exporter stderr:")
            print(result.stderr)

            print_log(f"Godot exporter exit code: {result.returncode}")

            if result.returncode == 0:
                print_log(f"Resource export done: {output_path}")
            else:
                QMessageBox.warning(
                    self,
                    "Godot export failed",
                    result.stderr or result.stdout or "Unknown Godot export error"
                )

        except Exception as e:
            print_log(f"Godot export failed: {e}")
            QMessageBox.critical(self, "Godot export failed", str(e))
